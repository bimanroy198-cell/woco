...
def send_otp():
    ...
    # IP-based Rate limiting: Prevent a single IP from spamming random numbers
    client_ip = request.remote_addr
    recent_ip_requests = otp_collection.count_documents({
        "ip_address": client_ip,
        "requested_at": {"$gt": int(time.time()) - 300} # last 5 minutes
    })
    
    if recent_ip_requests > 10:
        return jsonify({"error": "Too many requests from this device. Please try again later."}), 429
    ...
    otp_collection.update_one(
        {"mobile": mobile},
        {"$set": {"otp": otp, "expiration": expiration, "requested_at": current_time, "ip_address": client_ip}},
        upsert=True
    )
