...
def publish_release():
    try:
        ...
        # In the manual GitHub upload architecture, the frontend sends the direct URL and the manually inputted hash
        final_apk_url = request.form.get("apk_url")
        sha256 = request.form.get("sha256")
        
        if not final_apk_url or not sha256:
            return jsonify({"success": False, "message": "APK URL or SHA-256 missing"}), 400
        ...
        log_admin_action(session.get("api_admin_email"), "PUBLISH_APK", f"woco_v{version_name}.apk", True, {"version": version_name})
        return jsonify({"success": True, "message": "Release published successfully!"})
